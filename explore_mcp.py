...
async def main() -> None:
    ...
    try:
        ...
        for t in tools:
            ...
            if props:
                if list(props.keys()) == ["arguments"]:
                    # Flatten wrapper-style tools
                    print(f"       Params: arguments (object)", file=sys.stderr)
                else:
                    for pname, pinfo in props.items():
                        ptype = pinfo.get("type", "any")
                        print(f"       Params: {pname} ({ptype})", file=sys.stderr)
        ...
        # ── Call with agent_id: still NOT_AUTHORIZED ⚠️ ─────────────────
        print(f"\n{'─'*60}", file=sys.stderr)
        print(f"  🧪 Calling chat_post_message WITH agent_id (still blocked)",
              file=sys.stderr)
        print(f"{'─'*60}\n", file=sys.stderr)
        ...
        if agent_id:
            result = await client.call_tool("chat_post_message", {
                "arguments": {
                    ...
                    "_agent_id": agent_id,  # ← passed in the payload
                },
            })
        else:
        ...
        if is_error:
            print(file=sys.stderr)
            print(f"  ⚠️  WHY THIS FAILS — The Agora AuthMiddleware checks the MCP", file=sys.stderr)
            print(f"     session_id (auto-generated by FastMCP during stdio handshake)", file=sys.stderr)
            print(f"     against the agent registry. The session_id is a random UUID,", file=sys.stderr)
            print(f"     NOT the agent_id from register. Even if we pass _agent_id in", file=sys.stderr)
            print(f"     the arguments, the middleware never inspects tool arguments —", file=sys.stderr)
            print(f"     it only looks at the transport-level session.", file=sys.stderr)
            print(file=sys.stderr)
            print(f"  🔧 Reference: `reference/007-mcp-usability.md` describes the", file=sys.stderr)
            print(f"     fix — switch auth from session_id-based to argument-based.", file=sys.stderr)

        # ── Schema quality notice ───────────────────────────────────────
        print(f"\n{'─'*60}", file=sys.stderr)
        print(f"  📋 Schema quality notice", file=sys.stderr)
        print(f"{'─'*60}\n", file=sys.stderr)
        print(f"  The inputSchema for every tool says `arguments: object` —", file=sys.stderr)
        print(f"  completely useless for an LLM trying to discover the API.", file=sys.stderr)
        print(f"  Real parameter names, types, descriptions are all missing.", file=sys.stderr)
        print(file=sys.stderr)
        print(f"  🔧 Reference: `reference/007-mcp-usability.md` describes the", file=sys.stderr)
        print(f"     fix — explicit JSON schemas in ToolDef + proper FastMCP wiring.", file=sys.stderr)
